...
Returns `Promise<ThreadListItem>`. Throws `PollingTimeoutError` if attempts are exceeded. Polling also stops when the thread reaches `canceled` or `requires_action` (see [Pending user actions](#pending-user-actions)).
